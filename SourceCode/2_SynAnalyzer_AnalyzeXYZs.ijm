...
// *** USER PRESETS ***
// Default batch path
defBP = "/Users/joyfranco/Partners HealthCare Dropbox/Joy Franco/JF_Shared/Data/WSS/BatchAnalysis/SynAnalyzer_DemoBatch/";
...
bmCols = newArray("ImageName","ImInitialized?", "AnalysisStatus", "genThumbnailsStatus", "genArraysStatus",
				  "reviewArraysStatus", "AvailXYZData", "ZStart", "ZEnd", 
				  "Voxel Width (um)", "Voxel Depth (um)", 
				  "Synaptic Marker Channels", "Terminal Marker Channels");
cols = lengthOf(bmCols);
// LUT ASSIGNMENTS FOR USER ADJUSTED TIF
// The channels below correspond to the channel assignments in the original raw image
// C1 = Ribbon, C2 = PSD, C3 = CALB2, C4 = tdT
defMPICH = newArray("C1","C2","C3","C4");  // paired channel name for the LUT above
// The assignments below refer to the LUTs that will be assigned to each of the channels
// above to generate a tif of the original image. They are matched based on index array.
// C1 = c2 (green), C2 = c6 (magenta), C3 = c3 (blue), C4 = c7 (yellow)
defMPILUT = newArray("c2","c6","c3","c7"); 
// DEFAULT CHANNELS and LUTs TO USE FOR THUMBNAIL GENERATION
// Caution: these channels are based on the channel ordering in the saved tif, which is not guaranteed
// to match the original image. When ImageJ saves the TIF, it orders the channels based on the name
// of their LUT, following the order shown in the "Merge Channels" window (i.e., in this case this will
// be C1 = green, C2 = blue, C3 = magenta, C4 = yellow
// Default synapse channels: Ribbon = green, PSD = magenta
defSynCh="[1,3]";
defSynLUT = newArray("c2","c6");
defSynColors = newArray("Green", "Magenta");
// Default terminal channels: PSD = magenta, tdT = yellow
defTerCh="[3,4]";
defTerLUT = newArray("c6","c7");
defTerColors = newArray("Magenta", "Yellow");
// DEFAULT EXTENSIONS TO ADD TO THUMBNAILS
...
// GET USER CHOICE FOR HOW TO PROCEED -- FX FOR CHOOSING ANALYSIS MODE
...
function runAnalysis(mode, batchpath){
	...
	while (batch != "Exit") {
		...
		if(fStatus == "Acceptable"){
			print("File "+imName+" is valid and has been registered.\nProceeding with analysis.");
			...
			// 3.2.5 Ask user about updating status
			Table.open(batchpath+sdAna+fBM);
							...
							if ((xPos >= bbXZ) && (xPos <= bbXO)) { 
								if ((yPos >= bbYZ) && (yPos <= bbYT)) {
									if ((slZ >= slStart) && (slZ <= slEnd)){
										...
										// Edit the code below to ensure that enough z slices are used in the max
										// projections
										...
										//setFont("SansSerif",3, "antialiased");
									    //setColor(255, 255, 0);
										//drawString(".", ((tnW/vxW)/2), ((tnW/vxW)/2));
									    setFont("Monospaced",8,"antialiased");
									    setColor(255, 255, 255);
...
function reviewArrays(batchpath, imName, imIndex){
	...
	if(Dialog.getRadioButton()=="Yes"){
		...
		tnSteps = newArray("Synaptic","Terminal");
		tnCount = lengthOf(tnSteps);
		txtArr = newArray("Enter 'Synapse', 'Unpaired','Garbage', or a custom flag as needed.",
		...
		for (i = 0; i < tnCount; i++) {
			...
			for (j = 0; j < xyzFCount; j++) {
				...
				if(File.exists(batchpath+sdSA+imName+"."+tnSteps[i]+"Array."+availXYZ[j]+"."+fs+".png")){
					...
					waitForUser("*PROCEED WITH MANUAL THUMBNAIL SCORING*\n"+
